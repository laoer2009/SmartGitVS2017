...
namespace DomainEventExample
{
    ...
    /// <summary>
    /// 事件总线：EventBus是实现事件的核心，在这版里，它支持异步事件机制,使用Task实现
    /// ，所以它需要运行在.net4.5平台之上
    /// 发布与订阅处理逻辑
    /// 核心功能代码
    /// </summary>
    public class EventBus
    {
        ...
        public void Subscribe<TEvent>(Action<TEvent> eventHandlerFunc)
            where TEvent : class, IEvent
        {
            Subscribe<TEvent>(new Action<TEvent>(eventHandlerFunc));
        }
        ...
        /// 发布事件，支持异步事件
        /// </summary>
        /// <typeparam name="TEvent">某个领域对象：为了实现某个业务，而创建的实体类，它里面有事件所需要的数据，它继承了IEvent</typeparam>
